    ...
    std::shared_ptr<service::am::IApplet> CreateApplet(
        ...
        switch (appletId) {
            ...
            case AppletId::LibraryAppletOfflineWeb:
                return std::make_shared<OfflineWebApplet>(state, manager, std::move(onAppletStateChanged), std::move(onNormalDataPushFromApplet), std::move(onInteractiveDataPushFromApplet), appletMode);
            default:
